...
gen_data2 = function(n,p1,p2, type, a, rho = 0.3){
  ...
  if(type==1){#sparse H1
    Y = X[,1]+X[,2]+(a/sqrt(5)*(X[,p1+1]+X[,p1+2]+X[,p1+3]+X[,p1+4]+X[,p1+5]))^2+epsilon
  }
  if(type == 2){#dense H1
    Y = X[,1]+X[,2]+(a/sqrt(floor(p2/2))*rowSums(X[,(p1+1):(p1+floor(p2/2))]))^2+epsilon
  }
  Z = X[,1:p1]
  W = X[,(p1+1):p]
  return(list(Z,W,Y))
...
func = function(xxx,n,p1,p2,c=0.8){
  ...
  # data = gen_data1(n,p1,p2,1,0.20)#H1 sparse
  data = gen_data1(n,p1,p2,2,0.20)#H1 dense
  # #Example A2
  # data = gen_data2(n,p1,p2,1,0)#H0
  # data = gen_data2(n,p1,p2,1,0.50)#H1 sparse
  # data = gen_data2(n,p1,p2,2,0.50)#H1 dense
  # #Example A3
...
for(n in c(600, 800)){
    ...
    for(i in 1:iter){
      ...
      result[i,1] = mean( (1 - pchisq(ans[1:i,1], df = 1)) < 0.05 )
      result[i,2] = mean( (1 - pchisq(ans[1:i,2], df = 1)) < 0.05 )
      result[i,3] = mean( ans[1:i,3] < 0.05  )
      result[i,4] = mean( ans[1:i,4] < 0.05 )
      result[i,5] = mean( (1 - pnorm(ans[1:i,5])) < 0.05 )
      result[i,6] = mean( (1 - pnorm(ans[1:i,6])) < 0.05 )
      result[i,7] = mean( ans[1:i,7] < 0.05 )
      #result[i,8] = mean(ans[1:i,8]<qnorm(0.05))
      #result[i,9] = mean(ans[1:i,9]<0.05)
      if(i%%1==0){
        cat('d2, n: ',n,' time:',i,' result: ',result[i,],'\r')
      }
    }
    # result[iter,]
    write.csv(ans,paste0('Example_A1_power_d2_ans_n_',n,'.csv'))
}
#write.csv(t(result[iter,]),paste0('result_n',n,'.csv'))
# [1] 0.796 0.852 0.864 0.972 0.848 0.996 0.212
...
for(n in c(600, 800)){
    ...
    for(i in 1:iter){
      ...
      #result[i,9] = mean(ans[1:i,9]<0.05)
      if(i%%1==0){
        cat('s2, n: ',n,' time:',i,' result: ',result[i,],'\r')
      }
    }
    # result[iter,]
    write.csv(ans,paste0('Example_A1_power_s2_ans_n_',n,'.csv'))    
}
